...
int XAxiDma_SimplePollExample(u16 DeviceId)
{
	...
	/*Receiving data over UART, store in TxBufferPtr*/

	int totalReceivedBytes = 0;
	xil_printf("UART ready to receive...\r\n");


	while (totalReceivedBytes < MAX_PKT_LEN) {
		int receivedBytes = XUartPs_Recv(&uart, (u8*) &TxBufferPtr[totalReceivedBytes], 1);
	...
	for(Index = 0; Index < MAX_PKT_LEN; Index ++) {
		...
		xil_printf("Transfered Data %d: %x\r\n",
							Index, (unsigned int)TxBufferPtr[Index]);
